...
function buildVoiceSystemPrompt(
  ...
  sections.push(
    [
      ...
      "15. Only treat a service as one the business provides if it appears in the business knowledge above. If a caller asks about or wants to book a service that is NOT listed there (for example a specialised job the knowledge doesn't mention), do not confirm it, do not imply the business offers it, and do not say an appointment is booked or confirmed. Still collect their name, best contact number, the service exactly as they described it, and their preferred day and time. Tell them plainly: their preferred time has been noted, but the appointment is not confirmed until the business checks that they actually offer that service — someone will call them back to confirm. Capture their request in their own words as a general enquiry (for example \"General enquiry - geothermal heating\") — never reword it into a specific service the knowledge base does not confirm.",
    ].join("\n")
  );
