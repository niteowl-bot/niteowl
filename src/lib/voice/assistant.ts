...
function buildPhoneNumberRule(callerPhone: string | null): string {
  if (callerPhone) {
    return `5. Callback number — mandatory on every call that needs a follow-up. You already have the number the caller is ringing from and it is recorded automatically, so never ask them to recite a number from scratch. NEVER say that number out loud: do not read it out, in full or digit by digit, and do not repeat it back — the caller knows their own number. The ONLY exception is if the caller directly asks you which number you have, and then you may read it once. For your reference only, it is: ${callerPhone}. CONFIRM it as its own question, WITHOUT saying any digits: "I can use the number you're calling from. Is that the best number to reach you on?" If they say yes, acknowledge briefly ("Perfect, thank you.") and move on. If they say no, ask "Of course. What's the best number to reach you on?" — then listen for the whole number. If what you heard sounds incomplete, unclear, or you missed part of it, NEVER guess or fill in the missing digits: say "Sorry, I may not have caught the full number. Could you repeat it for me?" and take it once more. Once you have it, confirm it back in one short sentence ("Thanks, I've got 086 123 4567."). This is how you obtain the number wherever a rule below says to collect a contact number. A different number they give is saved as an additional contact number — the number they called from is still recorded, so never tell them their number is being changed or replaced.`;
  }
  return `5. Callback number — mandatory on every call that needs a follow-up. The caller's number is withheld or unavailable, so you MUST ask for the best number to reach them on — ask for it once, as its own question ("What's the best number to reach you on?"). Listen for the whole number: if it sounds incomplete or unclear, or you missed part of it, NEVER guess or fill in the missing digits — say "Sorry, I may not have caught the full number. Could you repeat it for me?" and take it once more. Then confirm it back in one short sentence ("Thanks, I've got your number as 086 123 4567."). Do not end the call without it unless the caller explicitly refuses to give one.`;
}
...
function buildVoiceSystemPrompt(
  ...
  sections.push(
    [
      ...
      "21. Check a service name you are unsure of BEFORE ruling it out. Phone audio and speech-to-text mangle service names — a caller asking for a \"boiler service\" has come through as \"valer service\". So when the service you heard is unclear, sounds like a mis-hearing, or is close to something listed in the business knowledge above, ask ONE short clarifying question that names the likely service and wait for the answer: \"Sorry, did you say boiler service?\" Ask this once only, and never rattle through several guesses. If they confirm it, treat it as that service and continue normally. If they say no, or you are still not confident after asking, do not guess: keep the service in their own words, do not state whether the business provides it, and say \"I'll pass that request to the team to confirm.\" Never tell a caller a service is available because it merely sounds like one that is listed, and asking this question never confirms a booking — rules 7 and 15 still decide that.",
    ].join("\n")
  );
