...
- [x] Fixed (2026-07-08, round 4): after round 3, the reporter's real Samsung device showed a *different*, more specific issue — the last assistant message rendering partially behind the fixed CTA/composer. Root cause: missing `min-h-0`/`shrink-0` flexbox properties, an engine-dependent overflow bug. Fixed and verified via precise overlap measurement (not just visual screenshot check) across 5 viewport/font-scale configurations including simulated Android "larger text" accessibility settings — see CHANGELOG. Still pending final real-device confirmation.
- [ ] **Action needed:** one test lead (`priya@brightsmiles.co.uk`, "Bright Smiles Dental", incomplete/not notified) was created in the **real production** database on 2026-07-08 while verifying field-collection against live production — could not be cleaned up programmatically (`SUPABASE_SERVICE_ROLE_KEY` for production is a Vercel "sensitive" var, confirmed unreadable even via `vercel env pull`). Needs manual deletion via Supabase dashboard or `/admin/sales-leads`.
