...
- [ ] **Action needed:** five test leads were created in the **real production** `sales_leads` table during 2026-07-08 verification work and could not be cleaned up programmatically (`SUPABASE_SERVICE_ROLE_KEY` for production is a Vercel "sensitive" var, confirmed unreadable even via `vercel env pull`) — need manual deletion via Supabase dashboard or `/admin/sales-leads`: (1) `priya@brightsmiles.co.uk`, "Bright Smiles Dental" (incomplete/not notified); (2) `claude-diag-test@example.com`, "Diagnostic Plumbing Ltd" (complete, notification sent — scripted production test); (3) `Dean@huy.com`, "Qwert Co", lead id `6dae1e56` (complete, notification sent — owner's real-device Chrome run); (4) `Ernie@samsiming.com`, lead id `252ef539` (complete, notification sent — owner's real-device Samsung run); (5) `Lupo@lup.com`, "Poiu", lead id `7fe58fbf` (incomplete/not notified — the failing Chrome run that exposed the round-8 root cause).
