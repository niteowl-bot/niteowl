...
- [ ] Set `VAPI_WEBHOOK_SECRET` + `VOICE_ENABLED=true` in the target environment (Vercel) only after the SQL has run — **half done (2026-07-10): the secret is set and redeployed, but `VOICE_ENABLED` is still off** — externally verified same day: both `/api/voice/*` routes answer 404 (dark), `/api/health` 200. Add `VOICE_ENABLED=true` in Vercel Production and redeploy
- [ ] **Un-assign the dashboard-built "Inbound AI Receptionist" assistant from the Vapi phone number** — an assistant was attached to the number on 2026-07-10; with one assigned, Vapi answers with that canned assistant instead of requesting Remy from our server URL, bypassing the Knowledge Base, lead capture, call records, and summary emails entirely. The number must have **no** assistant assigned — only the Server URL (see docs/VOICE_SETUP_RUNBOOK.md Step 3)
- [ ] Live end-to-end test call: Remy answers with KB knowledge, call row lands in `voice_calls`, lead created with source `voice`, owner receives the summary email — **not yet done**: the successful 2026-07-10 inbound test call was answered by the assigned dashboard assistant while the routes were still dark, so it proved the number/telephony only and never touched the integration (no `voice_calls` row, no lead, no email). Redo after the two items above
