...
- [ ] Delete the 2026-07-10 production test rows (voice_events, voice_calls, `voice` leads from the owner's test calls, and the linked conversations) once testing is finished — same hygiene as the earlier sales-leads cleanup
...
- [ ] Follow-up (cosmetic): both real calls opened with "Hi. For calling. How can I help you today?" — deterministic, so either the production `voice_settings.greeting` contains odd text (check `select greeting from voice_settings;` — NULL means our default is in play and this is just ASR of the TTS greeting) or the greeting needs re-recording/rewording
