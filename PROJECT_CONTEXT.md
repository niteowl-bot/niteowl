...
- **`structuredData: {}` suppresses the transcript fallback, NOT started.** `parseStructuredDetails` returns null only when `analysis.structuredData` is absent or not an object (`asRecord`), so an **empty or all-null** object yields a non-null `details` and `calls.ts`'s `if (!details)` never fires. The fallback extractor is skipped and every field stays null, on a call the transcript could have populated. Found while investigating PR #56 and **deliberately not touched by it** — it is a selection-logic question, not a producer-contract one, and changing it alters which calls run an LLM extraction. Keep it separate.
