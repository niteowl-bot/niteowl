...
- **Partial structuredData is never completed from the transcript, NOT started.** When the provider supplies *some* substantive fields, that object stays authoritative and the fallback extractor does **not** run to fill the gaps — so a call whose transcript plainly contains the email and the requested time can still record them as absent if `structuredData` held only, say, a name and a service. This is today's deliberate all-or-nothing producer selection, and PR #58 pins it with a test rather than changing it: running a second extractor to complete fields would put two independent readings of one call in play, the pattern the canonical-information architecture exists to remove. **Field-by-field completion is a separate architectural question and is NOT solved.**
  - **PR #60 did NOT close any of this.** It fixed the investigation's one *corruption* finding, on `service_address` alone. **Every information-LOSS finding remains open**: a spoken **email**, a **preferred_datetime**, an **urgency** and a **service** the transcript plainly contains can each still be recorded as absent when partial `structuredData` omits them — and a lost `service` additionally closes the booking gate, so a legitimate appointment becomes a `needs_review` lead. Do not read PR #60 as evidence that partial `structuredData` is handled.
  - **NEXT PLANNED INFORMATION-INTEGRITY STEP: deterministic spoken-email recovery for partial `structuredData`.** Recorded as the next investigation/implementation candidate and **NOT started** — no design is approved and no approach is committed to. Email is the candidate because `normaliseSpokenEmail` already exists as a deterministic guard and the owner's ability to reply is the most directly damaged by its absence. It must respect the same constraints as PR #60: no second AI extraction, no AI-output merge, no fallback extractor invoked to adjudicate one field, and no weakening of PR #58's producer-selection semantics.
