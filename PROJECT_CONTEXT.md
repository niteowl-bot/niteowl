...
- **F5 — NOT STARTED.** `service` is the last caller-supplied field this audit identified with **no dedicated deterministic guard**. Name has `resolveCallerName`, email has `normaliseSpokenEmail`, address has `resolveServiceAddress`, timing has `sanitisePreferredDatetime`; `extracted.service` passes through raw and decides `leads.service_needed`, the Knowledge Base check and the calendar event title. PR #48 removed its two prose fallbacks and PR #49 made it visible to the owner, so the field is now contained and observable — but it is not guarded. **With F4 complete, this is the next — and the last currently identified — canonical-information-integrity implementation item.** See the service-matching section below: the `requested_service` seam remains **deferred and not approved**, and the rejected approaches stay rejected unless a future investigation produces new evidence.

Canonical-information architecture (established across PRs #39, #42, #43, #45, #46, #47, #48, #49, #51):
