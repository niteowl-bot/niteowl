...
- **F4 STEP 3 — NOT STARTED.** Remove the seven factual labels (Name, Email, Callback number, the date, the time, Address, Issue) from `buildSummaryInstructions`, leaving the paragraph as narrative context only. Steps 1 and 2 have made every one of those facts available as a canonical row, so the labels are now duplication rather than the owner's only source — but the paragraph is **deliberately still present and still allowed to contradict** until this step ships. It is a **model-behaviour prompt change**: the suite can prove the instruction is present and coherent, never that the model obeys it, so **one live production call will be required to verify** — the same rule PR #34 and PR #40 established the hard way. Do not strip the labels before Step 2's rows are live in production, or the owner loses information.

- **F5 — NOT STARTED.** `service` is the last caller-supplied field this audit identified with **no dedicated deterministic guard**. Name has `resolveCallerName`, email has `normaliseSpokenEmail`, address has `resolveServiceAddress`, timing has `sanitisePreferredDatetime`; `extracted.service` passes through raw and decides `leads.service_needed`, the Knowledge Base check and the calendar event title. PR #48 removed its two prose fallbacks and PR #49 made it visible to the owner, so the field is now contained and observable — but it is not guarded. See the service-matching section below: the `requested_service` seam remains **deferred and not approved**, and the rejected approaches stay rejected.

Canonical-information architecture (established across PRs #39, #42, #43, #45, #46, #47, #48, #49):

**ONE CANONICAL FACT → MANY RENDERINGS.** Never several independent readings of the same call, hoping they agree.

- every caller-supplied field is resolved **once**, at the single convergence point in `toExtractedLead` (`voice/calls.ts`), by a deterministic guard: `resolveCallerName`, `normaliseSpokenEmail`, `resolveServiceAddress`, `sanitisePreferredDatetime`, plus `resolveCallbackUrgency` alongside
...
- the **provider-generated narrative remains contextual prose and is NOT an authoritative fact source.** It is kept in `voice_calls.summary`, in `leads.message`, and in the owner email beneath the rows, for review and audit. It decides nothing: PR #48 removed it as an extraction input and as a service fallback
