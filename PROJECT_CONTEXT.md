...
- **Partial structuredData is never completed from the transcript, NOT started.** When the provider supplies *some* substantive fields, that object stays authoritative and the fallback extractor does **not** run to fill the gaps — so a call whose transcript plainly contains the email and the requested time can still record them as absent if `structuredData` held only, say, a name and a service. This is today's deliberate all-or-nothing producer selection, and PR #58 pins it with a test rather than changing it: running a second extractor to complete fields would put two independent readings of one call in play, the pattern the canonical-information architecture exists to remove. **Field-by-field completion is a separate architectural question and is NOT solved.**
  - **PR #60 did NOT close any of this**, and **PR #62 closed exactly one field of it.** PR #60 fixed the investigation's one *corruption* finding, on `service_address`; PR #62 recovered **email**. **Every other information-LOSS finding remains open**: a **preferred_datetime**, an **urgency** and a **service** the transcript plainly contains can each still be recorded as absent when partial `structuredData` omits them — and a lost `service` additionally closes the booking gate, so a legitimate appointment becomes a `needs_review` lead. Do not read either PR as evidence that partial `structuredData` is handled.
  - **NEXT: no field is approved.** Email went first because `normaliseSpokenEmail` already existed as a deterministic guard and the customer being silently unreachable was the sharpest consequence. The remaining fields are **not** a queue to work through by default: each needs its own investigation, because each needs its own deterministic provenance rule and none of them has one yet. `preferred_datetime` in particular decides a booking, so recovering it wrongly is a corruption path, not a loss one.
  - **EMAIL is now the ONE exception, shipped by PR #62 (merged `c2d48b7`, deployed 2026-09-04) — see the entry below.** Every other omitted field still behaves exactly as this item describes. PR #62 did **not** change producer selection: the fallback extractor is still not invoked for a partial payload, and the recovery happens later, at the email convergence point, from the caller's own turns.
...
- every caller-supplied field is resolved **once**, at the single convergence point in `toExtractedLead` (`voice/calls.ts`), by a deterministic guard: `resolveCallerName`, `resolveCallerEmail`, `resolveServiceAddress`, `sanitisePreferredDatetime` and `resolveRequestedService`, plus `resolveCallbackUrgency` alongside. **All five caller-supplied fields are now guarded**; PR #54 closed the last gap. PR #62 replaced the email guard's normaliser-only form with `resolveCallerEmail`, which still calls the unchanged `normaliseSpokenEmail` first — **three of the five now read the transcript as evidence in their own right** (name, service address, email), and the other two remain null-in/null-out
...
**PARTIAL PROVIDER OUTPUT MUST NOT SILENTLY ERASE RELIABLE CALLER INFORMATION — but the fix must never become "two independent AI readings hoping they agree".** Field-level recovery requires a **deterministic provenance and authority rule**, and PR #62 is the first implementation of one, for email alone. **A field without such a rule stays unrecovered**, and the longer-term direction remains a single canonical extraction/provenance architecture, which is **not** started.
