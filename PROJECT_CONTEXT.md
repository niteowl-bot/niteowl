...
- **PR #39 — the email-derived caller name, merged, deployed and LIVE-PRODUCTION VERIFIED 2026-09-01.** This closes the "NEW, OPEN" caller-name item recorded against the PR #37 verification call, where the caller gave **"Ernesto"** and the owner email's structured **Caller** field read **"Ernie Sephora"** — a person who does not exist, whose name is the local part of the email address spoken later in the same call.
  ...
  - **Live production verification, 2026-09-01 — PASS.** A real call closed this out:
    - the caller explicitly gave the name **"Ernesto"**
    - the owner email's structured **Caller** field displayed **"Ernesto"**
    - it did **not** display the previous fabricated email-derived name
    - the generated summary also identified the caller as **Ernesto**, so the two surfaces agreed
    - **`Callback urgency: Urgent — no specific day or time given`** was still present, so PR #35's behaviour survived the change
    - **no false booking outcome appeared**, so PR #37's behaviour survived the change
  ...
  - **The same call exposed two SEPARATE findings, recorded below as open.** They are **not** part of PR #39 and are deliberately not bundled into it: PR #39 passed its own verification independently of both.

Deferred and non-blocking (do **not** pick these up as part of other work):

- **NEW, OPEN — FINDING A: email collection was skipped on the PR #39 verification call (observed 2026-09-01).** On that call Remy collected the caller's **name**, **address** and **callback telephone number**, but **never asked for an email address**. The owner email therefore correctly displayed **`Email: Not provided`** — the rendering is right; the *collection* is what did not happen. The immediately preceding urgent/no-specific-time call (the PR #35 and #37 verification call) **did** ask for an email, on the **same prompt**.

  **Established by read-only source analysis** (no code changed, no production transcript read — see the caveat below):
  - **There is no state machine.** The entire collection sequence is prompt text in `src/lib/voice/assistant.ts`. The live tool surface is exactly **`endCall` and `check_availability`** (`src/lib/voice/vapi.ts:360-362`); neither collects a field and neither sequences the call. So this cannot be a control-flow defect — **there is no control flow to defect.** It is a prompt-behaviour matter.
  - **The prompt contains two different required-field lists, and the call sat between them.** Rule 5's **COMPLETION GATE** applies to *"an appointment or service request"* and requires *service, calendar date, time, name, **email**, confirmed callback number, and the address*. Rule 13's **CALLBACK** list is shorter and says outright: *"An email and a service address are not required for a callback — ask only if the caller gives you a reason to."*
  - **Three separate rules push an urgent, timeless service request onto the callback track.** Rule 11's closing buckets *"CALLBACK (rule 13), **or a call that is urgent or needs a human**"* together. Rule 12 says an urgent caller gets their details taken *"and assure them of a callback"*. Rule 6, when urgency is all the caller gave, prescribes *"I'll note that and ask the team to ring you as early as they can"* — callback-shaped wording inside the appointment path. **Nothing in the prompt says which of the two lists governs a service request that has become urgent and has no time.**
  - **This also explains the address.** The address was collected even though rule 13 does not require it, which is consistent with rule 13's *"ask only if the caller gives you a reason to"* (a burst pipe at the caller's premises is such a reason) — i.e. a **callback-track call with the address asked for anyway**, not a rule 5 call with email dropped.
  - **Email is optional everywhere downstream.** No code requires it. `sendCallSummaryEmail` has **no structured Email field at all** — `Email: Not provided` comes from the generated summary paragraph, whose instructions require all seven labels and an explicit `"Not provided"`. So nothing downstream failed; the field was simply never obtained.

  **Test-coverage gap.** `tests/voiceConversation.test.mjs` pins **both** lists and their divergence — *"the gate names every required field, email and address included"* (line ~576) and *"the callback checklist is shorter, and rule 5's gate says so"* (line ~1157). **No test exercises the case where the two lists collide**: an urgent service request with no timing. Every prompt test is a `assert.match` against the prompt STRING; none drives a conversation. This is the same shape of gap that let PR #34 ship broken — the tests assert the text says the right thing, not that the right thing happens.

  **NOT established.** Which of the two paths the model actually took on the live call is **unproven** — reading the production `voice_calls` transcript and `structured_data` was **blocked by the auto-mode permission classifier**, twice. Until that transcript is read, the account above is a source-level explanation consistent with the evidence, **not a confirmed root cause**. **No code has been changed and none should be until the transcript confirms the path.**

- **NEW, OPEN — FINDING B: address recognition and confirmation on the PR #39 verification call (observed 2026-09-01).** The caller intended **`81 Oakland Drive`**. The conversation produced incorrect intermediate interpretations — **`K e 1 Auckland Drive`** and **`A c 1 Oakland Drive`** — before the caller explicitly corrected it to **`81 Oakland Drive`**, which is what the final owner-email summary carried.

  **Established by read-only source analysis**, separating the layers as required:
  - **Speech-to-text recognition — the origin, and it is provider-side.** Both bad forms mangle the *house number* `81` into letter sequences (`K e 1`, `A c 1`) while the street name stays broadly intact. That is a transcriber artefact, not a parsing one. **This repository configures no transcriber at all**: `buildVoiceAssistantConfig` emits `language`, `voiceId`, `maxDurationSeconds`, `firstMessage`, `systemPrompt`, `structuredDataSchema`, `summaryInstructions` and `serverUrl` — there is **no transcriber, model, keyword or spelling-boost setting anywhere in `src/`**. The transcriber lives in Vapi configuration, which is explicitly out of scope.
  - **Conversational confirmation — the prompt is arguably mis-tuned for house numbers.** Rule 5 step 7 tells Remy to query only an uncertain **street name** (*"Sorry, was that Birch Drive?"*), and rule 2 forbids reading the whole address back. Rule 7 requires digits spoken aloud to be read back, but that rule is about the **callback number**, not the house number. **A garbled house number therefore falls between the two rules** — `Oakland Drive` sounded fine, so step 7's street-name check would not fire on the part that was actually wrong.
  - **Correction handling — worked.** Rule 10 (latest correction replaces, immediately and everywhere) and rule 5 step 7's whole-address read-back are both pinned by tests (`tests/voiceConversation.test.mjs` lines ~216, ~943). The caller's correction was accepted and the final value was the corrected one.
  - **Extraction — worked.** `service_address` is a first-class extraction field in both the provider schema (`assistant.ts`) and the transcript fallback (`extraction.ts`), and both say *"if they corrected it during the call, use only the corrected version"*.
  - **Persistence and rendering are two INDEPENDENT paths that are never compared.** The lead's copy is `metadata.service_address`, written by `recordLeadCallDetails` from `details.service_address` (`calls.ts:766`). The `Address:` line the owner reads is in the **generated summary paragraph** — `sendCallSummaryEmail` has **no structured Address field**. **Only the summary line was observed as correct.** Whether `leads.metadata.service_address` also holds `81 Oakland Drive` has **NOT been verified** — that check needs the production read that was blocked.
  ...
  **NOT established.** The lead-record value, and therefore whether the two address paths agree. **Do not implement a speculative address parser and do not change Vapi/provider configuration.**
