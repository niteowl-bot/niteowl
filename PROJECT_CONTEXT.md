...
- **FINDING B is CLOSED — fixed by PR #42 (merged 2026-09-01), and extended by PRs #45, #46, #47 and #60.** It is kept in full below because the layer-by-layer analysis is the reusable part, and because the correction it originally proposed turned out to be the wrong one — a record worth keeping. **A mangled HOUSE NUMBER was accepted without question (PR #39 verification call, 2026-09-01). ORIGIN PROVEN from the call transcript.** The caller intended **`81 Oakland Drive`**. Layer by layer:
  ...
  **CLOSED, and NOT by the correction this entry originally proposed.** The three paragraphs that stood here — "NOT verified", "Test-coverage gap" and "STILL OPEN after PR #40" — plus a "Smallest safe correction (NOT implemented, awaiting approval)" describing a **prompt-only** addition to rule 5 step 7, were all overtaken by shipped work. They are replaced rather than left standing, because each asserted something that is no longer true. What actually closed it:
  ...
  - **PR #60** (`a07a06a`) fixed the **authority** of that one decision, which PRs #42–#47 never questioned: a provider address that merely looked well formed was returned before the transcript was read at all. See the service-address authority rule below.
...
- **Service-address authority IS CLOSED — shipped by PR #60, merged `a07a06a` and deployed 2026-09-04.** The highest-severity finding of the Partial structuredData Information-Integrity Investigation, listed here only so a reader scanning the open items does not go looking for it; the standing rule it produced is the service-address authority rule below. **It is the only investigation finding implemented so far — the rest remain open, below.**
  - **Two defects, and both were CORRUPTION rather than omission.** Every other item from that investigation loses information; these two put a *wrong* value in front of a real-world action, which is worse, because a plausible address reads as correct on every surface that shows it.
  - **Defect 1 — a well-formed provider address was never checked against the caller.** `resolveServiceAddress` consulted the transcript only when the candidate was absent or looked like transcription noise. A candidate that merely passed the noise test was returned before any caller evidence was read, so **"syntactically well formed" was silently treated as "authoritative"** — two different properties. Caller `81 Oakland Drive` + provider `12 Meadow Court` recorded **12 Meadow Court**, on the lead, the owner email and the calendar event location, with the booking succeeding.
  - **Defect 2, found in PRE-MERGE REVIEW of the first fix — a same-street house-number conflict.** The place comparison uses alphabetic tokens, so `81 Oakland Drive` and `12 Oakland Drive` shared every place word and were read as one address, leaving the provider's number authoritative. **Proven on the real path**: the booking succeeded and the event location read `12 Oakland Drive` while the caller had said 81 — a van to a stranger's door. It also **silently defeated the caller's own spoken correction**, the strongest caller evidence the module has. *(Recorded deliberately: the first fix passed its own 19-test suite and full validation and was still incomplete. Review found this, not the tests.)*
  - **Neither "provider wins" nor "transcript wins" was defensible.** The transcript's digits are exactly what the 2026-09-01 call mangled twice; the provider's are what this review caught. Both policies were rejected against the actual code, and the resolver now answers **CORRECT INFORMATION OR SAFE UNCERTAINTY** instead of choosing whichever source happened to win first.
  - **No new machinery.** Both fixes are in `src/lib/voice/addressIntegrity.ts` at the existing convergence point. **No second AI extraction, no second model call, no AI-output merge, no fallback extractor used to adjudicate a field, no geocoding, no external lookup, no fuzzy or probability scoring, no model adjudication, no provider-specific logic.** Provider independence is preserved: the rule reads two strings and the caller's own turns.
  - **Validation.** Focused **26/26**; directly relevant regressions **361/361**; full suite **1418 pass / 0 fail** across 238 suites; `tsc` clean; scoped lint 0; repo-wide lint unchanged at 11. Mutation-verified on both fixes — removing the different-place override fails 7, "transcript always wins" fails 6, and shared-token overlap instead of containment fails 1; on the numeric rule, removing the conflict branch fails 4, returning the spoken value instead of refusing fails 2, removing correction awareness fails 2, and removing missing-number recovery fails 1. **No live call was required**, and the PR #43 distinction is why: this is a deterministic code guard driven end-to-end through the real `processCallEnded`, so the suite genuinely closes it. A model-behaviour correction still could not be.
  - **A mutation that reported clean was a FALSE NEGATIVE, and it is recorded because it generalises.** One multi-line search string silently failed to match against this repository's **CRLF** working tree, so the mutation was never applied and the suite passed. Re-applied properly, it failed 2. **A mutation that changes nothing proves nothing** — verify the edit landed before trusting a green run.
  - **Production closeout, 2026-09-04.** Normal two-parent merge commit `a07a06a` (parents `43b3589` and `e001f0f`); local `main` equals `origin/main`; the automatic production deployment `dpl_Fd6Q76AkZwWaJ2SnQoD2o8TfQFox` reached **READY** carrying the `niteowlhq.com`, `www` and `git-main` aliases; `/api/health` returned **HTTP 200** `{"status":"ok","database":"ok"}` and the homepage **HTTP 200**. The feature branch was deleted locally and remotely **after** that verification. No live call; no production business or customer data read or modified.
  - **Deployment correspondence was NOT SHA-verified.** `vercel inspect` exposed **no Git-source metadata** for this deployment — no commit SHA, branch or source field. Identification rests on it being the new production deployment created three seconds after the merge and carrying those aliases, **not** on a SHA comparison. That is the same evidence limit recorded for PRs #54 and #58, and weaker than the direct read PR #40 was able to make.
- **Partial structuredData is never completed from the transcript, NOT started.** When the provider supplies *some* substantive fields, that object stays authoritative and the fallback extractor does **not** run to fill the gaps — so a call whose transcript plainly contains the email and the requested time can still record them as absent if `structuredData` held only, say, a name and a service. This is today's deliberate all-or-nothing producer selection, and PR #58 pins it with a test rather than changing it: running a second extractor to complete fields would put two independent readings of one call in play, the pattern the canonical-information architecture exists to remove. **Field-by-field completion is a separate architectural question and is NOT solved.**
  - **PR #60 did NOT close any of this**, and **PRs #62 and #66 have closed exactly one field each.** PR #60 fixed the investigation's one *corruption* finding, on `service_address`; PR #62 recovered **email**; PR #66 recovered **preferred_datetime**. **The remaining information-LOSS findings are open**: an **urgency** and a **service** the transcript plainly contains can each still be recorded as absent when partial `structuredData` omits them — and a lost `service` additionally closes the booking gate, so a legitimate appointment becomes a `needs_review` lead. Do not read any of these PRs as evidence that partial `structuredData` is handled.
  - **NEXT: no field is approved.** Email went first because `normaliseSpokenEmail` already existed as a deterministic guard and the customer being silently unreachable was the sharpest consequence; timing went second because it is the one field Remy's core promise cannot function without, and because the investigation established that the recovery is *evidence selection*, not datetime parsing — `parseDatetimeToIso` already turns phrases into instants. The remaining fields are **not** a queue to work through by default: each needs its own investigation, because each needs its own deterministic provenance rule and neither of them has one yet.
  - **SERVICE RECOVERY IS ALSO V1.1/LATER (classified 2026-09-07).** Under the canonical rule the answer is **no**: the owner is told **correctly** — with a requested time present they get the *REQUIRES REVIEW* block plus caller, number, email, address, requested time, transcript and narrative — so the enquiry is not missed and the human safety net does its job, unlike the returning-customer overwrite below it. And the fix is **not deterministic-safe**: F5's whole guarantee is that the result is a subsequence of the model's own candidate, and with no candidate there is nothing to constrain, so recovery would mean *identifying* a service from raw speech — the `requested_service` architecture that is **deferred and not approved**, and the approaches its rejected list already forbids. A wrongly recovered service is **corruption** reaching the engineer's job title, the KB gate (which has a known fail-**open** `requiredMatches` false positive) and the customer's confirmation email. **Do not reopen it because PR #62 and PR #66 built adjacent machinery** — neither supplies the missing piece.
  - **URGENCY WAS INVESTIGATED (2026-09-07) AND ITS RECOVERY IS DEFERRED TO V1.1/LATER.** Applying the canonical rule — *"Does this stop a normal paying customer from reliably using Remy V1?"* — the answer is **no**, and the reasoning is recorded so it is not re-litigated: the loss **fails safe** (nothing is fabricated and nothing misrouted), **no workflow consumes urgency** — there is no priority queue, routing, SLA or capacity effect, and `organisations.emergency_mode_enabled` is an unrelated business-hours setting — the lead is still created in every ordinary shape, and the owner still receives a per-call summary carrying the caller's number, the transcript and a narrative the prompt asks to state the reason it is urgent. What is lost is **one advisory row**. That is a different class from PR #60 (a van to the wrong door), PR #62 (a booked customer left unreachable) and PR #66 (a booking the caller asked for not proceeding). **PR #68 shipped the investigation's independent Part (a) only** — see below. **Part (b), transcript-based urgency recovery, is NOT started and is NOT a V1 blocker.**
  - **EMAIL and REQUESTED TIMING are now the two exceptions, shipped by PR #62 (merged `c2d48b7`, deployed 2026-09-04) and PR #66 (merged `bd5853a`, deployed 2026-09-07) — see the entries below.** Every other omitted field still behaves exactly as this item describes. Neither PR changed producer selection: the fallback extractor is still not invoked for a partial payload, and each recovery happens later, at that field's convergence point, from the caller's own turns.
- **Partial-structuredData EMAIL recovery IS CLOSED — shipped by PR #62, merged `c2d48b7` and deployed 2026-09-04.** The first partial-FIELD recovery, listed here only so a reader scanning the open items does not go looking for it; the standing rule it produced is the caller-email evidence rule below. **It closed one field. It did not close partial `structuredData`.**
  - **The loss, proven on the real path.** `normaliseSpokenEmail` is a NORMALISER — null-in/null-out — so `normaliseSpokenEmail(details.email)` resolved to null whenever a partial payload omitted the field, on a call whose transcript plainly carried the address the caller spelled out. The transcript already reached `toExtractedLead` and was read there for the caller's **name** and **service address**; email was the field with evidence available and no reader for it. Replayed before the fix: `lead.email` null, `lead.status` **booked**, one calendar event created, the owner's **Email** row absent, and the only message sent going to the owner.
  - **Why that is worse than an empty field.** `sendBookingConfirmationEmails` is guarded by `if (customerEmail)`, so the customer confirmation was **not failed — it was never attempted**. A booking completed and the customer was told nothing, with no error anywhere to notice.
  - **The authority rule, in order.** Normalise the provider's value **first**; if it survives, keep it and **do not read the transcript at all** — no comparison, no conflict resolution. Only when the normalised structured email is **absent** is deterministic caller-turn evidence read. A malformed provider value that normalises to null falls into that same absence path deliberately, rather than inventing a separate authority class for it.
  - **This is NOT "the transcript wins", and NOT a merge.** A present, usable provider email is never overridden, even when the transcript holds a different address — **structured-versus-transcript conflict is deliberately OUT OF SCOPE**, because unlike the service address there is no deterministic way to say which of two valid addresses is wrong and no corruption path was demonstrated. No model call, no network, no fallback extractor, no second reading, no field-wise merge of two AI outputs. `normaliseSpokenEmail` is imported **unchanged**: the new module decides what to OFFER it and never widens what counts as valid.
  - **THE REJECTED APPROACH IS THE REUSABLE PART.** A greedy span locator was prototyped during the investigation and **manufactured well-formed but WRONG addresses in both directions** — shortest-first turned `john dot smith at gmail dot com` into **`smith@gmail.com`**, longest-first absorbed surrounding speech into the local part. A wrong address here is *deliverable*: a stranger receives the confirmation and the real customer still hears nothing. **Never locate an email by enumerating substrings until one validates.** Candidates are whole clauses bounded by the caller's own commas and sentence ends, so a local part cannot be cut through the middle; both corruption shapes are pinned as tests.
  - **Validation.** Focused **39/39**; relevant regressions **448/448**; full suite **1457 pass / 0 fail** across 245 suites; `tsc` clean; scoped lint 0; repo-wide lint unchanged at 11. Six mutations, source restored after each: removing transcript recovery fails 20; allowing assistant turns fails 1; overriding a present structured email fails 3; removing last-wins fails 2; reverting to a shortest-first span search fails 21; removing the answer-prefix strip fails 1. No live call was required — a deterministic guard driven end-to-end through the real `processCallEnded`, the PR #43 distinction.
  - **TWO MUTATIONS WERE INITIALLY INERT, AND THAT IS RECORDED BECAUSE IT GENERALISES.** Allowing assistant turns, and removing the answer-prefix strip, both reported clean against the first fixtures — the tests could not tell the mutated code from the real code. The gap was in the tests, not a pass. Distinguishing cases were added (an assistant read-back phrased with the same wording the self-declaration cue recognises; a comma-less `No john at example dot com`, realistic because speech-to-text punctuation is unreliable) and both mutations then bit. **A mutation that reports clean is a claim about the tests, not about the code.**
  - **Production closeout, 2026-09-04.** Normal two-parent merge `c2d48b7` (parents `fd09ebb` and `6289418`); local `main` equals `origin/main`; the automatic production deployment `dpl_8Dp6qygPpkH5Krv5MnH4q2PnyVXu` reached **READY** carrying the `niteowlhq.com`, `www` and `git-main` aliases; `/api/health` returned **HTTP 200** `{"status":"ok","database":"ok"}` and the homepage **HTTP 200**. The feature branch was deleted locally and remotely **after** that verification. No live call; no production business or customer data read or modified.
  - **Deployment correspondence was NOT SHA-verified.** `vercel inspect` exposed **no Git-source metadata** for this deployment. Identification rests on it being the new production deployment created two seconds after the merge and carrying those aliases — **not** a SHA comparison. Same limit as PRs #54, #58 and #60.
  - **The customer-confirmation coverage gap is NOT closed by this.** PR #62 proved the recovered address reaches the **input** the `if (customerEmail)` guard reads; the send itself happens inside `after()`, which the voice harnesses do not flush. That item stays open below.
- **Partial-structuredData REQUESTED-TIMING recovery IS CLOSED — shipped by PR #66, merged `bd5853a` and deployed 2026-09-07.** The second partial-FIELD recovery, listed here only so a reader scanning the open items does not go looking for it; the standing rule it produced is the caller-timing evidence rule below. **It closed one field. It did not close partial `structuredData`.**
  - **The loss, proven on the real path and already pinned in the repository.** `sanitisePreferredDatetime` is a FILTER — null-in/null-out — so `sanitisePreferredDatetime(details.preferred_datetime)` resolved to null whenever a partial payload omitted the field, on a call whose transcript plainly carried the day the caller was asked for and gave. The transcript already reached `toExtractedLead` and was read there for the caller's **name**, **service address** and (since PR #62) **email**; timing was the field with evidence available and no reader for it. `tests/voiceStructuredDataEmpty.test.mjs` test D had recorded the loss as *expected* behaviour — *"a field with no deterministic guard is still NOT completed"*. PR #66 gives it the guard.
  - **Why that matters, and how it differs from the other integrity work.** This is a **downgraded booking, not a wrong one**. With no timing there is no instant, `isBookingConfirmed` cannot confirm, no calendar event is written, and the owner sees neither a Requested-appointment row nor a booking-status block. It fails SAFE — nothing is invented and no wrong time reaches anyone — which is why it is a reliability defect rather than a corruption one, and why it still blocks V1: the caller said when they wanted the visit and gets a request nobody can act on.
  - **The authority rule, in order.** Sanitise the provider's value **first**; if a real timing survives, keep it byte for byte and **do not read the transcript at all** — no comparison, no conflict resolution. If the sanitiser returns **URGENCY**, that is an ANSWER and not an absence: the caller was asked when and said "as soon as possible", so recovery is **blocked** and the urgency reaches the owner exactly as before (the PR #35 rule, kept). Only when the field is genuinely **absent or blank** is deterministic caller-turn evidence read.
  - **EVIDENCE SELECTION, NOT DATETIME PARSING — and that is the whole reason this is small.** The recovered value is the caller's **phrase**, taking the identical downstream path a provider phrase takes: `parseDatetimeToIso`, with the organisation's timezone, DST and `snapToNamedWeekday` correction. Nothing in the guard parses, resolves, completes, repairs or converts a datetime, so **no timezone, DST, availability, calendar or booking-policy semantics changed**. `sanitisePreferredDatetime` and `parseDatetimeToIso` are imported **unchanged**.
  - **A CLOCK WITH NO DAY RECOVERS NOTHING, and this is the rule most worth keeping.** *"AI: what time on Friday? / User: 3 PM."* carries a caller-spoken clock whose DAY came from the assistant; recovering `3 PM` alone would resolve against today and book the wrong day. Evidence must pin a day, a date or a relative day of its own. A bare day part (*"the afternoon"*) and a spelled ordinal with no month (*"the twelfth"*) are refused for the same reason.
  - **Caller turns only, and anchored.** Evidence is read only from an answer to an explicit assistant timing question, or from an explicit caller cue whose wording supplies the left boundary. **Assistant turns are never read** — including when the assistant's own offer is phrased *exactly* like a recognised caller cue (*"How about Tuesday at 2?"*), where **only the speaker separates a suggestion from evidence**. A bare acknowledgement — "yes", "correct", "that's right" — supplies nothing, because it confirms the assistant's proposal rather than stating a time. The **last** reliable anchored caller value wins, so a correction is deterministic ordering rather than inference.
  - **Ordinary speech is not a booking preference.** A house number, a phone number, a duration, a quantity and an unanchored mention of a day while describing the problem all recover nothing, and each is pinned by a test.
  - **Resolved ONCE, at the convergence point.** `processCallEnded` computes the timing and hands it to every consumer — the lead's `preferred_datetime` (through `toExtractedLead`), the owner's urgency decision, the booking-status gate and the requested-time row — so the two former call sites cannot derive different answers. The parameter is **required, with no default**, deliberately: a default would let a future caller omit it and silently lose the field, which is the very defect this fixes.
  - **No new machinery.** `resolveRequestedDatetime` (`src/lib/voice/datetimeIntegrity.ts`) is synchronous, deterministic, model-free and network-free. **No second AI extraction, no second model call, no fallback extractor (still never invoked for a partial payload — PR #58 untouched and still pinned), no record-level merge, no provider adjudication, no prompt, schema, database, migration, provider or configuration change.**
  - **Validation.** Focused **52/52**; relevant regressions **579/579**; full suite **1509 pass / 0 fail** across 251 suites; `tsc` clean; scoped lint 0; repo-wide lint unchanged at 11. A real-path replay through `parseVapiWebhook` → `processCallEnded` proves booking and calendar behaviour is **IDENTICAL** to the same call with the provider supplying the value — same instant, same status, same owner rows, same single datetime parse. Mutation-verified, source restored after each: removing recovery fails 19; reading assistant turns fails 3; overriding a present structured value fails 4; removing the urgency block fails 2; first-wins instead of last fails 1 at **each of the three ordering sites**; dropping the anchoring requirement fails 1; accepting a clock with no day fails 2; removing the address rejection fails 1. **No live call was required** — a deterministic guard driven end-to-end through the real `processCallEnded`, the PR #43 distinction.
  - **ONE MUTATION IS INERT, AND IT IS DOCUMENTED IN THE MODULE RATHER THAN QUIETLY DROPPED.** The sanitiser pass-through inside the reader is unreachable by construction today: every day anchor is also a `CONCRETE_TIMING` token, so a candidate reaching it can never be urgency-only. It stays as the invariant it states — widening the anchors is exactly the change that would make it load-bearing. **Two other mutations were initially inert and the tests were fixed, not the claim**: reading assistant turns, and first-wins ordering, both reported clean until distinguishing cases were added (an assistant offer phrased with the same wording the cue recognises; two answers to two separate timing questions). **A mutation that reports clean is a claim about the tests, not about the code** — the PR #62 lesson, repeating.
  - **Two existing expectations updated, neither weakened.** Test D's `preferred_datetime` is now recovered rather than null, and its extractor-never-runs assertion beside it is unchanged and still passes. Test A3's fixture had been reusing a transcript carrying *"Thursday 20 August at 3 PM"* while describing a caller who gave **no** time — a mismatch that was invisible while nothing read the transcript for timing, and that a real timing now correctly resolves by suppressing the urgency row. Its fixture is corrected to the call it describes; **its assertion is unchanged**, and no unrelated assertion was touched.
  - **Production closeout, 2026-09-07.** Normal two-parent merge `bd5853a` (parents `0a436e7` and `837caa7`); local `main` equals `origin/main`, fast-forwarded only; the automatic production deployment reached **READY** carrying the `niteowlhq.com`, `www` and `git-main` aliases; `/api/health` returned **HTTP 200** `{"status":"ok","database":"ok"}` and the homepage **HTTP 200**. The feature branch was deleted locally and remotely **after** that verification. No live call; no production business or customer data read or modified.
  - **Deployment correspondence was NOT SHA-verified.** `vercel inspect` exposed **no Git-source metadata** for this deployment. Identification rests on it being the new production deployment created two seconds after the merge and carrying those aliases — **not** a SHA comparison. Same limit as PRs #54, #58, #60 and #62.
- **Urgency REPRESENTABILITY is CLOSED — shipped by PR #68, merged `c390f53` and deployed 2026-09-07. Urgency RECOVERY is NOT, and remains V1.1/later.** Listed here only so a reader scanning the open items does not go looking for it, and so the two halves are never confused. **PR #68 is not an open V1 item and is not a V1 blocker.**
  - **The collapse it ended.** `parseStructuredDetails` wrote `urgent: data.urgent === true`, which turned **four different situations** into a single `false`: an explicit `false`, a field a partial payload omitted, a blank value, and a malformed one (`"true"`, `1`, `"yes"` — the schema asks for a boolean, and nothing enforces what arrives). Nothing downstream could tell *"the caller is not urgent"* from *"we were not told"*, so nothing downstream could ever act on the difference.
  - **The contract now, on `VoiceExtractedDetails.urgent` (`boolean | null`):** `true` — the provider explicitly said the caller was urgent; `false` — the provider explicitly said they were not; `null` — absent, blank, or not a boolean. **A malformed value is `null`, never coerced**: reading the string `"true"` as urgency would invent a provider statement that was never made, the same rule `asString` applies to blank text.
  - **BEHAVIOUR-NEUTRAL BY CONSTRUCTION, and that is the point.** Every production read site tests `=== true` — the urgency resolution and the needs-review gate in `calls.ts`, and the emptiness test in `vapi.ts` — so **`null` behaves exactly as the old `false` did**. The one bare truthiness test was made explicit rather than left relying on `null` being falsy. No owner-facing claim was created, no booking, calendar, lead-routing or prioritisation behaviour was introduced, and nothing was inferred from absence.
  - **WHAT WAS DELIBERATELY NOT BUILT.** No transcript read, no caller-turn evidence parsing, no speaker analysis, no keyword scanning, no model call, no `urgencyIntegrity.ts`, no `resolveCallerUrgency`, no urgency routing, queue, SLA, capacity change, booking priority or emergency workflow. **This is representational hardening, nothing more** — it makes future recovery work *possible* by giving absence a representation; it does not begin it.
  - **Three files:** `src/lib/voice/types.ts`, `src/lib/voice/vapi.ts` and a new `tests/voiceUrgencyRepresentability.test.mjs`. No prompt, schema, database, migration, provider, configuration, calendar, availability, `preferred_datetime`, `callbackTiming`, email or dashboard change. The transcript fallback extractor keeps its two-state output **on purpose**, and the reason is recorded on the type: it runs only when the provider said nothing at all, so *"we were not told"* is not a state it can be in.
  - **Validation.** Focused **21/21**; relevant regressions **406/406**; full suite **1530 pass / 0 fail** across 254 suites; `tsc` clean; scoped lint 0; repo-wide lint unchanged at 11. The real-path half **proves** the neutrality claim rather than asserting it: `false` and `null` produce an identical owner email, identical stored metadata, identical lead status and identical booking outcome; a real timing still suppresses the urgency row in **every** state (PR #35); PR #66 timing recovery still works; and a lead is still created for every urgency state. Mutation-verified, source restored and `diff`-confirmed identical after each: restoring the two-state collapse fails 6; coercing truthy values instead of refusing them fails 4; letting `null` count as substance in the emptiness test fails 3.
  - **A MUTATION HELPER THAT REPORTS "ANCHOR NOT FOUND" IS DOING ITS JOB.** The first run failed to apply all three mutations, because the helper matched **LF** anchors against this repository's **CRLF** working tree — the same false-negative shape PR #60 hit, and the reason the helper now tries both line endings. Had it silently reported a pass, three meaningless greens would have been recorded as evidence.
  - **Production closeout, 2026-09-07.** Normal two-parent merge `c390f53` (parents `c209c11` and `a2c483f`); local `main` equals `origin/main`, fast-forwarded only; the automatic production deployment reached **READY** carrying the `niteowlhq.com`, `www` and `git-main` aliases; `/api/health` returned **HTTP 200** `{"status":"ok","database":"ok"}` and the homepage **HTTP 200**. The feature branch was deleted locally and remotely **after** that verification. **No live call was required** — deterministic type/boundary hardening with no intended runtime behaviour change. No production business or customer data read or modified.
  - **Deployment correspondence was NOT SHA-verified.** `vercel inspect` exposed **no Git-source metadata**. Identification rests on it being the new production deployment created two seconds after the merge and carrying those aliases — **not** a SHA comparison. Same limit as PRs #54, #58, #60, #62 and #66.
- **The RETURNING-CUSTOMER BOOKING OVERWRITE is CLOSED — shipped by PR #70, merged `8833896` and deployed 2026-09-07. It is NO LONGER an open V1 blocker.** Chat and widget only; voice was never affected. Listed here so a reader scanning the open items does not go looking for it, and because the checklist and `docs/ARCHITECTURE.md` C3 still describe it as open — **this entry is the current record.** The standing rule it produced is the cross-conversation matching rule below.
  - **The defect.** `findOpenLeadForCapture` layer 2 matched a lead by **email OR phone**, across every conversation the customer had ever had, with **no time bound** — and `MERGEABLE_STATUSES` included `booked`. The update path then read whatever it found as the appointment being discussed: an already-`booked` lead given a different requested time is a RESCHEDULE, so `rescheduleAppointmentOnCalendar` **moved the existing Google event**. A returning customer booking a SECOND appointment — the ordinary case for every trade Remy serves — had their FIRST one moved on top of it.
  - **Why it was a V1 blocker, under the canonical rule.** Jane books Tuesday; a month later, in a new chat, she books Friday. The Tuesday event is dragged to Friday, the Tuesday row is overwritten, **no confirmation email is sent** (that send tests `existing.status !== "booked"`), and the reply still reports **"booked"**. A confirmed commitment destroyed while reporting success — and, unlike the information-loss items, **with no human safety net**: nothing is flagged for review, because the system believes it performed a routine reschedule.
  - **The fix: IDENTIFYING THE PERSON IS NOT IDENTIFYING THE APPOINTMENT.** Within ONE conversation those coincide, so **layer 1 keeps the full status list including `booked`** and an in-session reschedule still moves the event. Across conversations they do not, so layers 2 and 3 use `CROSS_CONVERSATION_MERGEABLE_STATUSES` — the same list **minus `booked`**, derived from `MERGEABLE_STATUSES` so the two cannot drift — and the second booking takes the ordinary new-lead path. `MERGEABLE_STATUSES` itself is unchanged.
  - **THE TRADE, DELIBERATE AND PINNED BY A TEST.** A genuine reschedule started in a NEW conversation now creates a second lead rather than moving the first. That is the safe direction — two visible records an owner can reconcile, against an appointment silently destroyed — and `/api/bookings/manage` remains the designed reschedule path, unchanged.
  - **Rejected, and recorded so they are not retried:** removing `booked` from `MERGEABLE_STATUSES` globally (breaks layer 1's legitimate in-conversation reschedule and `isBookingCompletedByContactUpdate`); a time bound on layer 2 (a threshold with no principled value — a second booking a week later is as legitimate as one a month later); and any appointment-identity redesign, which stays **deferred** (`docs/ARCHITECTURE.md` §P1).
  - **One production file.** `src/lib/leadCapture.ts` — one derived constant and two status filters. `PROTECTED_STATUSES`, the voice exemption, layer 1, the **calendar-rescheduling internals**, the **manage-booking flow**, calendar sync, availability, capacity, booking-slot logic, timezone/DST handling, prompts, schema, migrations, providers and configuration are all **untouched**, as is the existing provider/calendar architecture.
  - **Validation.** Focused **13/13**; relevant regressions **349/349**; voice **632/632**; full suite **1543 pass / 0 fail** across 256 suites; `tsc` clean; scoped lint 0; repo-wide lint unchanged at 11. The tests drive the real `capturePartialLead`, and the leads stub honours the `status=in.(…)` filter the code actually sends rather than deciding for itself what may match — **which is what makes the mutations meaningful rather than cosmetic**. Mutation-verified, source restored and `diff`-confirmed identical after each: putting `booked` back into **layer 2** fails 7 (including the calendar-event-not-moved test); removing `booked` from **layer 1** fails 2; putting `booked` back into **layer 3** fails 1.
  - **A MUTATION THAT REPORTED CLEAN WAS A TEST GAP, NOT A PASS.** The layer-3 mutation was initially inert because every scenario supplied contact details, so layer 3 was never reached. A no-contact-details test was added rather than the coverage being claimed, and it bites now — the same lesson PRs #60, #62 and #68 recorded.
  - **No live call was required** — a deterministic change replayed end-to-end through the real capture path.
  - **Production closeout, 2026-09-07.** Normal two-parent merge `8833896e409b4a7e6bccafc857e377a8d6f0fd5c` (parents `35fe88d` and `72717f5`); local `main` equals `origin/main`, fast-forwarded only; the automatic production deployment reached **READY** carrying the `niteowlhq.com`, `www` and `git-main` aliases; `/api/health` returned **HTTP 200** `{"status":"ok","database":"ok"}` and the homepage **HTTP 200**. The feature branch was deleted locally and remotely **after** that verification. No live call; no production business or customer data read or modified.
  - **Deployment correspondence was NOT SHA-verified.** `vercel inspect` exposed **no Git-source metadata**. Identification rests on it being the new production deployment created three seconds after the merge and carrying those aliases — **not** a SHA comparison. Same limit as PRs #54, #58, #60, #62, #66 and #68.
...
- **The customer booking-confirmation surface is not directly observed by any test, NOT started.** `leadCapture` passes the resolved `extracted.service` into `sendBookingConfirmationEmails`, but that email is dispatched inside `after()`, which the voice test harnesses do not flush — so the only surface the **customer** reads is asserted nowhere. This is a **pre-existing coverage gap across every voice suite**, surfaced by PR #56 rather than caused by it, and PR #56 deliberately did not assert it vacuously. Closing it means teaching the harness to flush `after()`, which is test-infrastructure work. **PR #62 did NOT close it** — it proved the recovered email reaches the `if (customerEmail)` guard's input, which is the value the send reads, but the asynchronous send itself is still asserted nowhere.
