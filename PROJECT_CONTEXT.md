...
- **Fallback extractor contract, NOT started and deliberately separate.** The transcript fallback extractor invites a short service summary and has historically used examples such as *"Boiler repair"*, which encourages paraphrasing, whereas the `structuredData` contract asks for the caller's own wording. **The two extraction contracts are still not aligned.** PR #54 provides deterministic downstream protection — a paraphrase that the caller did not support is now reduced or refused rather than stored — but the guard treats the symptom and the prompts remain unaligned. This is a **prompt / model-behaviour** follow-up, and the PR #34 and PR #40 lesson applies: a test can prove the instruction is present, only a live call can prove the model obeys it. **The prompt was not touched by F5.**
...
Service-integrity rule (closed by PR #54, merge commit `36805b8`, deployed 2026-09-03):
